...
const char* rtc_strchr(const char* str, const char* chs) {
...
int inet_pton_v4(const char* src, void* dst) {
    ...
    while (*src_pos != '\0') {
        ...
        result[found - 1] = (unsigned char)(value);
        src_pos = end_pos;
        if (*src_pos == '.') {
            // There's more.
            ++src_pos;
...
const char* inet_ntop_v6(const void* src, char* dst, socklen_t size) {
    ...
    if (runpos[0] == 1 && (maxpos == 5 ||
        ...
        *cursor++ = ':';
        if (maxpos == 4) {
            cursor += _snprintf(cursor, INET6_ADDRSTRLEN - 2, "ffff:");
        }
        const struct in_addr* as_v4 =(const struct in_addr*)(&(as_shorts[6]));
        inet_ntop_v4(as_v4, cursor,
            (socklen_t)(INET6_ADDRSTRLEN - (cursor - dst)));
    }
    else {
        for (int i = 0; i < run_array_size; ++i) {
            if (runpos[i] == -1) {
                cursor += _snprintf(cursor,
